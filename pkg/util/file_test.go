...
func TestReplaceFile(t *testing.T) {
	...
	t.Run("Non-existent dest", func(t *testing.T) {
		...
		err := util.CopyReplaceFile(dest, source, 0644, nil)
		assert.NoError(t, err)
	...
	t.Run("WriteReplaceFile writes content with correct permissions", func(t *testing.T) {
		basedir := t.TempDir()
		dest := filepath.Join(basedir, "dest")
		content := []byte("hello world")

		err := util.WriteReplaceFile(dest, content, 0640, nil)
		assert.NoError(t, err)

		expectContentAndPerm(t, dest, content, 0640)
	})
}
